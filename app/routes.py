...
async def get_notes(search: Optional[str] = Query(None, min_length=1), db: Session = Depends(get_db)):
...
async def update_note(note_id: str, note_update: schemas.NoteUpdate, db: Session = Depends(get_db)):
...
async def restore_note_version(note_id: str, version_id: str, db: Session = Depends(get_db)):
    """Restore a note to a specific version"""
    note, error = crud.restore_note_version(db=db, note_id=note_id, version_id=version_id)
    if error:

...
def get_note_versions(db: Session, note_id: str):
    return db.query(models.NoteVersionDB).filter(models.NoteVersionDB.note_id == note_id).order_by(models.NoteVersionDB.version.desc()).all()
...
def restore_note_version(db: Session, note_id: str, version_id: str):
    ...
    version_data = db.query(models.NoteVersionDB).filter(models.NoteVersionDB.id == version_id).first()
    if not version_data:
